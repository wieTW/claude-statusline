...
- **THEN** `resolve_theme` SHALL emit `.theme` from `~/.claude/settings.json` when that file is present and parseable with a usable value; SHALL emit the literal `dark` only when `~/.claude/settings.json` is present and parseable but its `.theme` is absent or empty; and SHALL emit an empty line (still exactly one line) when `~/.claude/settings.json` is missing or unparseable, in which case `load_palette` treats the empty non-`light` theme as dark via the `STYLE` catch-all and applies the `claude` default palette
...
<!-- @trace
source: statusline-theme-palette
updated: 2026-07-02
code:
  - lib/render.sh
  - lib/collect.sh
