# display-segments Specification

## Purpose

The display-segments capability defines the CONTENT and COLOUR of every base segment on the line — path, model, effort (with its recovered ultra/auto mode), the abnormal-only thinking indicator, git branch and dirty marker and diffstat, worktree, and session name — plus the inter-segment separator. It owns what each segment communicates and which palette role colours it, but not the terminal-width or degrade behaviour (that belongs to adaptive-layout) nor the palette's RGB values (that belongs to theme-palette). It also covers the NORM_THINKING config knob.
