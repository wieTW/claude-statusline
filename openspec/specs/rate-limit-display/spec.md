# rate-limit-display Specification

## Purpose

The rate-limit-display capability defines the base on-screen rate-limit segment for the five-hour and seven-day windows: the reset-countdown prefix, the remaining percentage (100 minus used, clamped to never below zero), the green-to-red colour ladder, the silence on a non-numeric value, and the compact form that drops the countdown. It is distinct from rate-limit-sync (which owns the value's cross-session reconciliation) and rate-burn-projection (which owns the depletion alarm that rides inside the five-hour segment).
